...
problem.bnds.pathf.ub = [ x(1,end)+dx; x(2,end)+dy; x(3,end)+dz; +inf; +inf; +inf; +inf ];
...
% -- solve 1 : rough mesh (more flexible)
solver_options.nSegment = 60;
solver_options.nOrder = 7;
solver_options.maxIter = 30;
[solution1,problem1] = feasibleTranscription(problem1,solver_options);

% -- pass data from solution
problem2 = problem;
problem2.traj.data = solution1.traj.data;

% -- solve 2 : fine mesh (more accurate)
solver_options.nSegment = 120;
solver_options.nOrder = 11;
solver_options.maxIter = 60;
[solution2,problem2] = feasibleTranscription(problem2,solver_options);
...
% axis equal;
title('Trajectory');
xlabel('x-axis [nd]'); ylabel('z-axis [nd]'); zlabel('z-axis [nd]');
...
text(problem.traj.data.x(1,1),problem.traj.data.x(2,1),problem.traj.data.x(3,1),'  t_0','HandleVisibility','off');
...
plot3(solution2.traj.data.x(1,:),solution2.traj.data.x(2,:),solution2.traj.data.x(3,:),'b');
plot3(solution2.traj.data.x(1,end),solution2.traj.data.x(2,end),solution2.traj.data.x(3,end),'*g','HandleVisibility','off');
text(solution2.traj.data.x(1,end),solution2.traj.data.x(2,end),solution2.traj.data.x(3,end),' target','Color','g','HandleVisibility','off');
legend('guess','feasible');
...
xlabel('time [nd]');
ylabel('mass [kg]');
plot(solution2.traj.data.t,solution2.traj.data.x(7,:));

subplot(3,1,2);
xlabel('time [nd]');
ylabel('thrust [N]');
hold on; grid on;title('Thrust');
plot(solution2.traj.data.t,solution2.traj.data.u(1,:));

subplot(3,1,3);
xlabel('time [nd]');
ylabel('thrust [N]');
hold on; grid on;title('Thrust Components');
plot(solution2.traj.data.t,solution2.traj.data.u(1,:).*solution2.traj.data.u(2,:));
plot(solution2.traj.data.t,solution2.traj.data.u(1,:).*solution2.traj.data.u(3,:));
plot(solution2.traj.data.t,solution2.traj.data.u(1,:).*solution2.traj.data.u(4,:));
legend('T_T','T_W','T_N');

% plot raw error distribution (constraints)
figure();
semilogy(abs(solution2.fval));
hold on; grid on;
